...
async def create_emergency(db: Session, data: EmergencyCreate, background_geocode: bool = False) -> Emergency:
    ...
    # Check for pending location in cache
    phone_key = normalize_phone(record.caller_phone)
    if phone_key and phone_key in pending_locations:
        loc_data = pending_locations.pop(phone_key)
        record.latitude = loc_data.latitude
        record.longitude = loc_data.longitude
        record.location_captured = True
        db.commit()
        db.refresh(record)
...
def get_emergencies_by_caller(db: Session, phone: str) -> list[Emergency]:
    ...
    return (
        db.query(Emergency)
        .filter(Emergency.caller_phone.like(f"%{phone_key}"))
        .order_by(Emergency.created_at.desc())
        .all()
    )
